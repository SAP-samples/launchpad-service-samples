    ...
    if (req.headers['x-approuter-authorization'] && parsedUrl.path === '/applications') {
      bsUtils.getHTML5Applications(req, function (err, html5Applications) {
        if (err) {
          ...
          if (html5Applications.errors && html5Applications.errors.length > 0 && html5Applications.errors.length < 50) {
            let failedDestinations;
            html5Applications.errors.forEach((error) => {
              failedDestinations = failedDestinations ? failedDestinations + ',' + error.destinationName : error.destinationName;
            });
            res.setHeader('x-failed-destinations', failedDestinations);
          }
          res.end(JSON.stringify(html5Applications.applications, null, 4));
        }
      });
    } else {
      resolve();
